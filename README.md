...
### 🔄 Intelligent Organization, Mapping & Versioning
...
- **`bfxpm save` (Conventional Commits)**: Commitizen-style interactive prompter ensuring standardized conventional commit messages.
- **`bfxpm bump` (Automated Releases)**: Automatically determines semver version increments from Conventional Commit history, bumps package versions (`pyproject.toml`, `conda/meta.yaml`, `__init__.py`), updates `CHANGELOG.md`, and tags the new release in one command.
...
> **Starting BfxPM**
> Start using `bfxpm` or `bfxpm --help` to see all the available commands.
